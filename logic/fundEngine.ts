...
export const calculateFundCompletely = (fundData: FundData): CalculatedFund => {
  ...
  if (personale2018_perArt33 > 0) { const importoArt67 = personale2018_perArt33 * VALORE_PROCAPITE_ART67_CCNL2018; incrementiStabiliCCNL.push({descrizione: `Incremento stabile CCNL (${VALORE_PROCAPITE_ART67_CCNL2018}€ pro-capite su personale 2018 per Art.33)`, importo: importoArt67, riferimento: RIF_ART67_CCNL2018, tipo: 'stabile', esclusoDalLimite2016: false, });}
  if (personaleNonDirigenteEQAttuale_perArt33 > 0) { const importoArt79b = personaleNonDirigenteEQAttuale_perArt33 * VALORE_PROCAPITE_ART79_CCNL2022_B; incrementiStabiliCCNL.push({descrizione: `Incremento stabile CCNL (${VALORE_PROCAPITE_ART79_CCNL2022_B}€ pro-capite personale non Dir/EQ per Art.33)`, importo: importoArt79b, riferimento: `${RIF_ART79_CCNL2022} lett. b)`, tipo: 'stabile', esclusoDalLimite2016: false, });}
  ...
  if (annualData.condizioniVirtuositaFinanziariaSoddisfatte && annualData.incentiviPNRROpMisureStraordinarie && annualData.incentiviPNRROpMisureStraordinarie > 0) { const limitePNRR = fondoBase2016_originale * LIMITE_INCREMENTO_PNRR_DL13_2023; const importoEffettivoPNRR = Math.min(annualData.incentiviPNRROpMisureStraordinarie, limitePNRR); risorseVariabili.push({descrizione: "Incremento variabile PNRR/Misure Straordinarie (fino a 5% del fondo stabile 2016 originale)", importo: importoEffettivoPNRR, riferimento: RIF_ART8_DL13_2023, tipo: 'variabile', esclusoDalLimite2016: true, });}
  
  const limiteArt23C2Modificato = fondoBase2016_originale + (incrementoDeterminatoArt23C2?.importo || 0);
  
  const ammontareSoggettoLimite2016_global = fondoBase2016_originale + incrementiStabiliCCNL.filter(c => !c.esclusoDalLimite2016).reduce((s,c)=>s+c.importo,0) + (adeguamentoProCapite.esclusoDalLimite2016 ? 0 : adeguamentoProCapite.importo) + (incrementoDeterminatoArt23C2 && !incrementoDeterminatoArt23C2.esclusoDalLimite2016 ? incrementoDeterminatoArt23C2.importo : 0) + (incrementoOpzionaleVirtuosi && !incrementoOpzionaleVirtuosi.esclusoDalLimite2016 ? incrementoOpzionaleVirtuosi.importo : 0) + risorseVariabili.filter(c => !c.esclusoDalLimite2016).reduce((s,c)=>s+c.importo,0);
  ...
  if (risorseDaDistribuire > 0) {
      ...
      if (importoRimanente < -0.005) { // Tolleranza per errori di arrotondamento
          checks.push({
              ...
              descrizione: "Superamento del budget nella Distribuzione Risorse",
              isCompliant: false,
              valoreAttuale: `€ ${totaleAllocato.toLocaleString('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
              limite: `€ ${risorseDaDistribuire.toLocaleString('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
              messaggio: `L'importo totale allocato supera le risorse disponibili di € ${Math.abs(importoRimanente).toLocaleString('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}.`,
          ...
          checks.push({
              ...
              descrizione: "Rispetto del budget nella Distribuzione Risorse",
              isCompliant: true,
              valoreAttuale: `€ ${totaleAllocato.toLocaleString('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
              limite: `€ ${risorseDaDistribuire.toLocaleString('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
              messaggio: `L'allocazione delle risorse rispetta il budget. Rimanenza: € ${importoRimanente.toLocaleString('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}.`,
              riferimentoNormativo: "Art. 80 CCNL 16.11.2022",
              gravita: 'info',
          });
      }
